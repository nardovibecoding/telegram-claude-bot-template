...
```bash
ssh <user>@<vps-ip> "crontab -l > /tmp/cron_backup && crontab -l"
# Add new entry:
ssh <user>@<vps-ip> "(crontab -l; echo '0 12 * * * cd ~/telegram-claude-bot && source venv/bin/activate && python script.py >> /tmp/start_all.log 2>&1') | crontab -"
```

### 5. Test manually
```bash
ssh <user>@<vps-ip> "cd ~/telegram-claude-bot && source venv/bin/activate && python script.py"
```

### 6. Verify flag file written
```bash
ssh <user>@<vps-ip> "cat ~/telegram-claude-bot/.last_<name>_run"
```
