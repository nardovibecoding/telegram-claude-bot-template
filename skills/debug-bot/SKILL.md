...
```bash
ssh <user>@<vps-ip> "tail -50 /tmp/start_all.log"
```

### 2. Filter for errors
```bash
ssh <user>@<vps-ip> "grep 'ERROR\|WARNING\|Traceback\|Exception' /tmp/start_all.log | tail -30"
```
...
# Then verify:
ssh <user>@<vps-ip> "sleep 8 && tail -10 /tmp/start_all.log"
```
