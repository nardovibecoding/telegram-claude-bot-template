...
```bash
ssh <user>@<vps-ip> "grep 'ERROR\|WARNING' /tmp/start_all.log | sort | uniq -c | sort -rn | head -20"
```

### 2. Get recent errors (last few hours only)
```bash
ssh <user>@<vps-ip> "grep 'ERROR\|WARNING' /tmp/start_all.log | tail -20"
```

### 3. Check for tracebacks
```bash
ssh <user>@<vps-ip> "grep -B2 'Traceback\|Exception\|Error' /tmp/start_all.log | tail -30"
```

### 4. Check restart frequency
```bash
ssh <user>@<vps-ip> "grep 'Starting\|stopped.*restarting' /tmp/start_all.log | tail -20"
```
