...
# VPS HEAD
ssh <user>@<vps-ip> "cd ~/telegram-claude-bot && git log --oneline -1"
```
...
ls -la ~/.claude/projects/-Users-bernard/memory/*.md

ssh <user>@<vps-ip> "ls -la ~/.claude/projects/-home-bernard/memory/*.md"
```
- Memory dirs differ: Mac uses `-Users-bernard`, VPS uses `-home-bernard`
...
# VPS MCP servers
ssh <user>@<vps-ip> "cat ~/.claude/settings.json | python3 -c \"import sys,json; print(sorted(json.load(sys.stdin).get('mcpServers',{}).keys()))\""
```
- Same server names should appear on both sides
- Paths will differ (Mac ~/ vs VPS ~/)
