...
```bash
ssh YOUR_VPS_USER@YOUR_VPS_IP "python3 -c \"
import json, time, os
...
```bash
ssh YOUR_VPS_USER@YOUR_VPS_IP "ls -la ~/telegram-claude-bot/.cookie_refresh_lock 2>/dev/null || echo 'No lock'"
```
...
# Or manually:
ssh YOUR_VPS_USER@YOUR_VPS_IP "cd ~/telegram-claude-bot && source venv/bin/activate && python send_xdigest.py"
```

### 5. Check logs for pipeline errors
```bash
ssh YOUR_VPS_USER@YOUR_VPS_IP "grep -i 'xcurat\|curator\|twikit\|twitter' /tmp/start_all.log | tail -20"
```
