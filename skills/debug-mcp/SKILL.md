...
```bash
# XHS
ssh <user>@<vps-ip> "curl -s http://localhost:18060/health"
...
```bash
# XHS logs
ssh <user>@<vps-ip> "tail -30 /tmp/xhs-mcp-py.log"

# Douyin logs
ssh <user>@<vps-ip> "tail -30 /tmp/dy-mcp-py.log"
```

### 3. Restart the service
```bash
# XHS
ssh <user>@<vps-ip> "systemctl --user restart xhs-mcp"

# Douyin
ssh <user>@<vps-ip> "systemctl --user restart douyin-mcp"
```

### 4. Verify after restart
```bash
ssh <user>@<vps-ip> "sleep 3 && curl -s http://localhost:18060/health"
ssh <user>@<vps-ip> "systemctl --user status xhs-mcp"
```

### 5. Check service status
```bash
ssh <user>@<vps-ip> "systemctl --user status xhs-mcp douyin-mcp"
```
