...
```
Expected: 8 persona bots (bot1, bot2, devasini, twitter, xcn, xai, xniche, reddit) + 1 admin_bot = 9 processes

### 2. If a specific bot is missing
```bash
# Check logs for crash reason
ssh YOUR_VPS_USER@YOUR_VPS_IP "grep '<bot_name>' /tmp/start_all.log | tail -20"
```
...
```bash
ssh YOUR_VPS_USER@YOUR_VPS_IP "pgrep -af start_all"
# If not running:
ssh YOUR_VPS_USER@YOUR_VPS_IP "cd ~/telegram-claude-bot && nohup ./start_all.sh >> /tmp/start_all.log 2>&1 &"
```
...
### 7. Verify recovery
```bash
ssh YOUR_VPS_USER@YOUR_VPS_IP "sleep 10 && pgrep -af 'run_bot\|admin_bot' && echo '---' && tail -5 /tmp/start_all.log"
```
