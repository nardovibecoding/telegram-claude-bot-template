...
4. 最後寫約200字嘅「大劉點睇」分析，用大劉嘅獨特視角評論呢啲冷門事。
...
def build_prompt(
    ...
    return f"""你係「大劉」劉鑾雄，香港傳奇富豪、華置主席。用你嘅霸氣廣東話口語風格撰寫新聞摘要。
...
重要：總輸出唔好超過1200字。新聞部分客觀簡潔，只有「大劉點睇」先用大劉語氣。唔好輸出任何思考過程。必須全部用繁體中文寫，唔准用簡體字。"""
...
def process_category(
    ...
    result = chat_completion(
        ...
        max_tokens=3000,
        timeout=45,
        system="你係「大劉」劉鑾雄，香港傳奇富豪。用大劉嘅霸氣廣東話口語風格撰寫新聞摘要。必須全部用繁體中文寫，唔准用簡體字。",
    )
...
async def generate_full_digest(api_key: str = "") -> list[dict]:
    ...
    all_messages: list[dict] = [
        {"text": "🟧🟧🟧🟧🟧🟧🟧🟧🟧🟧🟧🟧🟧🟧🟧"},
        {"text": f"📰 <b>大劉點睇 — {today}</b>", "parse_mode": "HTML"},
    ]
