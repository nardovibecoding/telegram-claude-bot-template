...
class TestCheck(unittest.TestCase):
    ...
    def test_visibility_public(self):
        self.assertTrue(self._call("gh repo edit <github-user>/foo --visibility public"))
    ...
    def test_extracts_repo_name_from_cmd_and_finds_home_dir(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            with patch.object(Path, "home", return_value=Path(tmp)):
                result = app._find_repo_path("gh repo edit <github-user>/my-repo --visibility public")
...
class TestActionIPFalsePositive(unittest.TestCase):
    ...
    def _run_action_on_content(self, filename, content, cmd="gh repo edit <github-user>/test-repo --visibility public"):
        with tempfile.TemporaryDirectory() as tmp:
            repo = Path(tmp)
            (repo / ".git").mkdir()
    ...
    def _run_with_topics(self, topics):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            with patch("auto_pre_publish._find_repo_path", return_value=repo), \
                ...
                return app.action("Bash", {"command": "gh repo edit <github-user>/test-repo --visibility public"}, {})
