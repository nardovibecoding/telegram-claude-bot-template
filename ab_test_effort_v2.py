...
Uses claude CLI (Claude Max) with different system prompts.
Allows tool use (web search) for questions that need real-time data.
"""
...
def call_claude(question, system_prompt, max_turns=3):
    ...
    cmd = [
        ...
        "--allowedTools", "mcp__fetch__fetch,WebSearch,WebFetch",
    ]
    ...
    if result.returncode != 0:
        return elapsed, f"ERROR: {result.stderr[:300]}", 0
    ...
    try:
        ...
        if is_error and not text:
            text = f"[Model tried tool use but hit max turns]"
        if not text and out_tokens > 0:
            text = f"[Generated {out_tokens} tokens but result empty — likely tool use response]"
    except (json.JSONDecodeError, KeyError):
        text = result.stdout[:2000]
        out_tokens = len(text.split())

    return elapsed, text, out_tokens
...
for i, question in enumerate(questions):
    ...
    # LOW effort
    print("  [LOW] Running...", flush=True)
    t_low, text_low, tok_low = call_claude(question, LOW_SYSTEM)
    print(f"  [LOW] {t_low:.1f}s | {tok_low} tokens | {len(text_low)} chars")

    time.sleep(2)

    # MEDIUM effort
    print("  [MED] Running...", flush=True)
    t_med, text_med, tok_med = call_claude(question, MED_SYSTEM)
    print(f"  [MED] {t_med:.1f}s | {tok_med} tokens | {len(text_med)} chars")
...
print("-" * 83)
print(f"{'AVG':<25} {total_low_t/5:>8.1f}s {'':>7} {total_med_t/5:>8.1f}s {'':>7} {(total_med_t-total_low_t)/5:>+8.1f}s")
...
for r in results:
    ...
    print(f"  {'-'*100}")
    for line in r['low_response'][:1000].split('\n'):
        print(f"    {line}")
    if len(r['low_response']) > 1000:
        print(f"    [...truncated at 1000 chars, total {len(r['low_response'])}]")

    print(f"\n  >>> MEDIUM EFFORT ({r['med_time']}s, {r['med_tokens']} tokens):")
    print(f"  {'-'*100}")
    for line in r['med_response'][:1500].split('\n'):
        print(f"    {line}")
    if len(r['med_response']) > 1500:
        print(f"    [...truncated at 1500 chars, total {len(r['med_response'])}]")
