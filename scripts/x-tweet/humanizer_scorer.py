...
def score_humanity(text: str) -> dict:
    ...
    if total >= 85:
        ...
        label = "AI fingerprint \u2014 full rewrite required \U0001f534"
...
def print_report(result: dict, label: str = "") -> None:
    ...
    if v["ai_word_hits"] > 0:
        ...
        print(f"  \U0001f534 AI vocabulary: {v['ai_word_hits']} hits \u2014 [{terms}]")
    else:
        print("  \u2705 No AI vocabulary detected")

    sv = s["sentence_variance"]
    if sv["std_dev"] < 5:
        print(f"  \U0001f534 Sentence rhythm robotic \u2014 std dev only {sv['std_dev']} (target: 8+)")
    elif sv["std_dev"] < 8:
        print(f"  \U0001f7e1 Sentence variance low \u2014 {sv['std_dev']} (target: 8+)")
    else:
        print(f"  \u2705 Sentence variance good \u2014 {sv['std_dev']}")

    pv = s["passive_voice"]
    if pv["passive_ratio"] > 0.3:
        print(f"  \U0001f534 Passive voice overuse \u2014 {pv['passive_pct']} of sentences")
    elif pv["passive_ratio"] > 0.2:
        print(f"  \U0001f7e1 Passive voice elevated \u2014 {pv['passive_pct']}")
    else:
    ...
    if hg["hedge_count"] > 2:
        ...
        print(f"  \U0001f534 Hedging overload \u2014 {hg['hedge_count']} phrases: [{terms}]")
    elif hg["hedge_count"] > 0:
        print(f"  \U0001f7e1 Hedging present \u2014 {hg['hedge_count']} phrase(s): {hg['flagged_phrases']}")
    else:
        print("  \u2705 No hedging detected")

    if hg["vague_authority_count"] > 0:
        print(f"  \U0001f7e1 Vague authority claims: {hg['vague_authority_count']} (e.g. 'studies show') \u2014 add citations")

    em = s["em_dashes"]
    if em["per_100_words"] > 3:
        print(f"  \U0001f7e1 Em-dash overuse \u2014 {em['em_dash_count']} in piece ({em['per_100_words']}/100 words)")

    pg = s["paragraph_variety"]
    if not pg.get("has_short_paragraphs"):
        print("  \U0001f7e1 No short paragraphs found \u2014 add some 1-2 sentence paragraphs for rhythm")
